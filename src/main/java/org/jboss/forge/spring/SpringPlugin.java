...
public class SpringPlugin implements Plugin
{
   ...
   /**
    * The 'setup' command is used to initialize the project as a simple Spring Web MVC project. For example, this
    * command will add the necessary dependencies to the project's POM.xml file.
    */
   @Command("setup")
   public void setupProject(final PipeOut out)
   {
      ...
      DependencyBuilder springContextSupport = DependencyBuilder
               .create("org.springframework:spring-context-support:3.1.0.RC1");
   ...
   /**
    * The 'persistence' command is used to configure the persistence layer for the application. Before executing 'spring
    * persistence', the Forge provided command 'persistence setup' should have been executed. Thus, the application's
    * persistence layer should already be mostly configured in META-INF/persistence.xml. This command should perform the
    * necessary steps to configure Spring persistence, e.g. an EntityManager JNDI look-up.
    */
   @Command("persistence")
   public void springPersistence(final PipeOut out)
   {
      ...
      webapp.attribute("xsi:schemaLocation",
   ...
   /**
    * The 'web-mvc' command configures Spring MVC in the application context. This command will be necessary to deploy
    * the application, once we have created MVC controllers.
    */
   @Command("web-mvc")
   ...
   /**
    * The 'mvc-from-entity' command creates a web controller from an entity. When the application is deployed on the
    * selected container, these controllers will provide a web view of the selected entity.
    * 
    * @throws FileNotFoundException
    */
   @Command("mvc-from-entity")
   public void generateFromEntity(final PipeOut out,
   ...
   public List<JavaResource> selectTargets(final PipeOut out, Resource<?>[] targets)
            throws FileNotFoundException
   {
   ...
   public JavaClass generateController(final JavaClass entity, final String mvcPackage)
   {
